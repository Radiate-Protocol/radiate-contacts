...
contract dLPZap is Ownable {
    ...
    AggregatorV3Interface public ethChainlink = AggregatorV3Interface(0x639Fe6ab55C921f74e7fac1ee960C0B6293ba612);

    /// @notice Chainlink oracle address for RDNT/USD
    AggregatorV3Interface public rdntChainlink = AggregatorV3Interface(0x20d0Fcab0ECFD078B036b6CAf1FaC69A6453b352);
    ...
    rDLP public constant rdLPVault = rDLP(0xC6dC7749781F7Ba1e9424704B2904f2F94D3eb63);
    ...
    function _zap(uint256 amountIn, bool direction) internal returns (uint256) {
        ...
        if (IERC20(path[0]).allowance(address(this), address(uniswapRouter)) == 0) {
        ...
        // Approvals
        if (IERC20(address(RDNT)).allowance(address(this), address(BALANCER)) == 0) {
            IERC20(address(RDNT)).approve(address(BALANCER), type(uint256).max);
        }
        if (IERC20(address(WETH)).allowance(address(this), address(BALANCER)) == 0) {
        ...
        if (IERC20(address(BALANCER_LP)).allowance(address(this), address(rdLPVault)) == 0) {
            IERC20(address(BALANCER_LP)).approve(address(rdLPVault), type(uint256).max);
        }
    ...
    function _estimateout(uint256 _amtIn, bool direction) internal view returns (uint256 tokensOut) {
